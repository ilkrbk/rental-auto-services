...
class Controller {
  ...
  findAllCars = async (req, res) => {
    const provider1 = (
      await axios.get("http://localhost:8081/rental/price-list/car")
    ).data;

    const provider2 = (
      await axios.get(
        `http://localhost:8082/rental/price-list/car?offset=${req.query.offset}`
      )
    ).data;
    ...
    await this.db.car
      .findAll({ limit: 5 })
      .then((data) => {
        res.send([...data, ...provider1, ...provider2]);
      })
      .catch((err) => {
        res.status(500).send({
          message: err.message || "Some error occurred while retrieving cars.",
